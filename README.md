...
- v1.0.5 adds a third scheduled task for scenarios with `Autologon` enabled or where `no password` is set.   
This task runs an intermediate script (`Wait for Desktop (Autologon and no-pw).exe`) which waits until the desktop has focus before launching `Sunshine WGC`. This is necessary because directly executing`Sunshine WGC` during a traditional login (with a password) causes streaming to fail, as `Sunshine WGC` would start before the user can enter their password. When the desktop is ready, you need to click on it with the mouse to launch of `Sunshine WGC`.  
